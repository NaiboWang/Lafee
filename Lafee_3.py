...
def train(batch_size=80, time_step=TIME_STEP, percentage=0.7, model=lafee_model, cell=c_BasicRNNCell,
    ...
    with tf.Session() as sess:
        ...
        f10 = open("origin_logs/" + summary_dir + "user_x.csv",'a',newline='')
        csv10 = csv.writer(f10)
        for data in np.asarray(train_x_origin).tolist():
            csv10.writerow(data)
        ...
        if restore: # 如果重新载入模型
            ...
            test_predict, test_output, test_state, mae, rmse, test_yy,a,b,c = TEST_model(sess, pred, satisfaction,aspiration, train_x, train_y, X,
...
def get_lf_data():
    ...
    test_predict, test_output, test_state,train_y_origin,train_x_origin = train(batch_size=400, model=lafee_model, summary_dir=summary_dir, num=num,
...
if __name__ == '__main__':
    ...
    # battle_top20 winning_bottom20 winning_top20 battle_bottom20
    PATH['input'] = "../../fb_dataset/fb_player_event_json/dataset_sorted_input/winning_top20/"
    import sys
    DATA = 0
    # PATH['input'] = sys.argv[1]
    # PATH['input'] = "origin_datas/all_origin/"
    FILES = [i for i in os.listdir(PATH['input']) if re.match(r'.*csv', i)]
    #print(FILES)
    ...
    if DATA:
        ...
        if type == "0":
            print("Lafee_train")
            train(batch_size=400,percentage=percent, model=lafee_model, summary_dir=summary_dir, num=num, mode = mode,restore = False)  # num为用户读入用户数目 mode1 按用户分割 mode0 按数据分割
        elif type == "1":
            print("LSTM_train")
            summary_dir = TIMESTAMP + '_lstm_model/'
            train(batch_size=400, percentage=percent,model=basic_rnn_model,cell=c_BasicLSTMCell,summary_dir=summary_dir, num=num,mode=mode, restore=False)  # num为用户读入用户数目 mode1 按用户分割 mode0 按数据分割
        else:
            print("RNN_train")
            summary_dir = TIMESTAMP + '_rnn_model/'
            train(batch_size=400, percentage=percent,model=basic_rnn_model,cell=c_BasicRNNCell,summary_dir=summary_dir, num=num,mode=mode, restore=False)  # num为用户读入用户数目 mode1 按用户分割 mode0 按数据分割